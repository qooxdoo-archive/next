...
describe("Xml", function() {
  ...
  it("ParseSerializeXml", function() {
    var doc = qx.xml.Document.create();
    assert.isTrue(qx.dom.Node.isDocument(doc));
    ...
    assert.isTrue(qx.dom.Node.isDocument(doc2));

    assert.equal(xmlStr, qx.xml.Element.serialize(doc2));
  });
  ...
  it("CreateDocument", function() {
    ...
    assert.equal('rss', doc.documentElement.tagName);
    assert.equal(0, doc.documentElement.childNodes.length);

    doc = qx.xml.Document.create("http://www.w3.org/1999/xhtml/", "html");
    assert.equal('http://www.w3.org/1999/xhtml/', doc.documentElement.namespaceURI);
  ...
  it("XPath", function() {
    ...
    assert.equal('<em id="toll">Kinners</em>', qx.xml.Element.serialize(em));

    assert.equal(qx.xml.Element.serialize(em), qx.xml.Element.serialize(qx.xml.Element.selectSingleNode(doc2, '//*[@id="toll"]')));

    assert.equal(qx.xml.Element.serialize(doc2.getElementsByTagName("body")[0]), qx.xml.Element.serialize(qx.xml.Element.selectSingleNode(doc2, '//body')));

    assert.equal(serializeArray(doc2.getElementsByTagName("em")), serializeArray(qx.xml.Element.selectNodes(doc2, '//em')));

    assert.equal(serializeArray(doc2.getElementsByTagName("em")), serializeArray(qx.xml.Element.selectNodes(doc2.documentElement, '//em')));
  });
  ...
  it("XPathNS", function() {
    ...
    if (navigator.userAgent.indexOf('Chrome') > 0 &&
      ...
      assert.equal(s4, fooStr);
    }
  });


  it("GetElementsByTagNameNS", function() {
    var xmlStr =
      "<\?xml version='1.0' " +
      "encoding='UTF-8'?>" +
    ...
    assert.equal(2, templates.length, "getElementsByTagNameNS for element nodes failed!");

    assert.equal("xsl:template", templates[0].tagName, "getElementsByTagNameNS for element nodes failed!");
  });


  it("SetAttributeNS", function() {
    ...
    if (node.getAttributeNS) {
      assert.equal("foo", node.getAttributeNS(namespaceURI, "qxid"));
    } else {
      assert.equal(namespaceURI, node.getAttributeNode("qxid").namespaceURI);
    }
  });


  it("GetAttributeNS", function() {
    var doc = qx.xml.Document.create("http://www.w3.org/1999/xhtml/", "html");
    var node = doc.createElement("a");
    var namespaceURI = "http://www.qooxdoo.org/";
    qx.xml.Element.setAttributeNS(doc, node, namespaceURI, "qxid", "foo");

    assert.equal('foo', qx.xml.Element.getAttributeNS(node, namespaceURI, 'qxid'));
  });


  it("CreateSubElementNS", function() {
    ...
    assert.equal(subElem, createdNode);
    assert.isTrue(subElem.parentNode == parent);
  });
});
