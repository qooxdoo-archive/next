 describe("q", function() {
  ...
  it("indexOf", function() {
    assert.equal(qxWeb().indexOf(), [].indexOf());
    assert.equal(qxWeb([document.documentElement, document.body]).indexOf(document.body),
      [0, 1].indexOf(1));
    assert.equal(qxWeb([document.documentElement, document.body]).indexOf(document.documentElement, 1),
      [0, 1].indexOf(0, 1));
    assert.equal(qxWeb("#sandbox").indexOf(qxWeb("#sandbox")[0], 5),
      [0].indexOf(0, 5));
    assert.equal(qxWeb("#sandbox").indexOf(qxWeb("#sandbox")[0], -5),
      [0].indexOf(0, -5));
    assert.equal(qxWeb([window, document.documentElement, window]).indexOf(window, -1),
      [0, 1, 0].indexOf(0, -1));
  });
});
