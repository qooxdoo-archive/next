  ...
  members : {
    ...
    */
    filter : function(keyword, columnName) {
      if (columnName) {
        ...
        columnName = qx.ui.control.Table.__allColumnSelector;
      }
    ...
    */
    __filterDom : function(keyword, columnName) {
      ...
      var filterFunc = columnName == qx.ui.control.Table.__allColumnSelector ? this.getRowFilter() : this.getColumnFilter(columnName);
      ...
      switch (rowSelection) {
        ...
        case "none":
          if (currentSelection != "none") {
            qxWeb("." + qx.ui.control.Table.__internalSelectionClass).remove();
          }
          break;
        ...
        case "multiple":
          if (currentSelection == "none") {
            ...
            qxWeb("." + qx.ui.control.Table.__internalSelectionClass + " input").setAttribute("type", "checkbox");
        ...
        case "single":
          if (currentSelection == "none") {
            ...
            qxWeb("." + qx.ui.control.Table.__internalSelectionClass + " input").setAttribute("type", "radio");
    ...
    */
    ...
    __getColumnMetaData : function() {
      ...
      for (var i = 0, l = cells.length; i < l; i++) {
        ...
        colName = this.__getColumnName(cell[0]) || qx.ui.control.Table.__getUID();
        if(!cell[0].getAttribute(qx.ui.control.Table.__dataColName)){
          cell.setAttribute(qx.ui.control.Table.__dataColName, colName);
        }

        data[colName] = {
          type: cell[0].getAttribute(qx.ui.control.Table.__dataColType) || "String",
          name: colName
        };
    ...
    * @return {qx.ui.control.Table} <code>this</code> reference for chaining.
    ...
    __addClassToHeaderAndFooter : function(footOrHead) {
      if (footOrHead && footOrHead.rows.length > 0) {
        if (footOrHead.rows.item(0).cells.length > 0) {
          if (!qxWeb(footOrHead.rows.item(0).cells.item(0)).hasClass(qx.ui.control.Table.__internalHeaderClass)) {
            qxWeb(footOrHead.rows.item(0).cells).addClass(qx.ui.control.Table.__internalHeaderClass);
          }
        }
      }
      return this;
    ...
    __registerEvents : function() {
      ...
      this.on("cellClicked", function(data) {
        if(data.cell && data.cell.hasClass(qx.ui.control.Table.__internalHeaderClass)){
    ...
    */
    __selectionRendered : function() {
      return qxWeb("." + qx.ui.control.Table.__internalSelectionClass).length > 0;
    },
    ...
    __detectClickedCell : function(e) {
      ...
      if (cell.hasClass(clazz.__internalSelectionClass)) {
        ...
        if (cell && cell.length > 0) {
          ...
          var colNumber = qx.ui.control.Table.__getIndex(cells, cell[0]);
    ...
    */
    ...
    __addSortingClassToCol : function(HeaderOrFooter, columnName, dir) {
      if (HeaderOrFooter && HeaderOrFooter.rows.length > 0) {
        ...
        var cell = qxWeb("["+qx.ui.control.Table.__dataColName+"='" + columnName + "'], #" + columnName);
    ...
    _compareNumber : function(x, y, direction) {
      x = qx.ui.control.Table.__isNumber(x) ? Number(x) : 0;
      y = qx.ui.control.Table.__isNumber(y) ? Number(y) : 0;
      if (direction == "asc") {
        return x - y;
    ...
    */
    __getColumnName : function(headerCell) {
      return headerCell.getAttribute(qx.ui.control.Table.__dataColName) || headerCell.getAttribute("id");
    },
    ...
    _compareDate : function(x, y, direction) {
      ...
      x = qx.ui.control.Table.__isDate(x) ? new Date(x) : new Date(0);
      y = qx.ui.control.Table.__isDate(y) ? new Date(y) : new Date(0);
    ...
    */
    __getSortingKey : function(cell) {
      return cell.getAttribute(qx.ui.control.Table.__dataSortingKey) || this.__getCellValue(cell);
    },
    ...
    __getDataRows : function() {
      ...
      for (var i = 0, l = rows.length; i < l; i++) {
        ...
        for (var j = 0, len = cells.length; j < len; j++) {
          cell = qxWeb(cells[j]);
          if (!cell.hasClass(qx.ui.control.Table.__internalCellClass)) {
            cell.addClass(qx.ui.control.Table.__internalCellClass);
          }
        }
    ...
    */
    __defaultColumnFilter : function(data) {
      var cell = data.columnName == qx.ui.control.Table.__allColumnSelector ? data.row : data.cell;
