...
describe("io.part.Part", function() {
  ...
  function createPart(name, pkgs, loader) {
    return new qx.io.part.Part(name, pkgs, loader)
  }


  it("load part with one package", function() {
    var pkg = new qx.test.io.part.MockPackage("1");
    var part = this.createPart("1", [pkg], this.__loader);
    assert.equal("initialized", part.getReadyState());

    var self = this;
    part.load(function(readyState) {
      self.resume(function() {
        assert.equal("complete", readyState);
        assert.equal("complete", part.getReadyState());
      })
    });

    assert.equal("loading", part.getReadyState());
    this.wait();
  });


  it("load part with several packages", function() {
    var packages = [
  ...
  it("delay loading of second package should preserve order", function() {
    var packages = [
      new qx.test.io.part.MockPackage("a"),
  ...
  it("one already loaded package should not be loaded again and preserve order", function() {
    var packages = [
  ...
  it("a currently loading package should not be loaded again and should preserve order", function() {
    var packages = [
      new qx.test.io.part.MockPackage("a", 0, false, "complete"),
      new qx.test.io.part.MockPackage("b"),
      new qx.test.io.part.MockPackage("c")
  ...
  it("error loading second package should set 'error' status on callback", function() {
    var packages = [
      new qx.test.io.part.MockPackage("a"),
    ...
    part.load(function(readyState) {
      self.resume(function() {
        assert.equal("error", readyState);
        assert.equal("error", part.getReadyState());
      })
    });
  ...
  it("loading a loaded part again should not reload the packages", function() {
    var packages = [
      new qx.test.io.part.MockPackage("a"),
      new qx.test.io.part.MockPackage("b"),
      new qx.test.io.part.MockPackage("c")
  ...
  it("loading an error part again should not reload the packages", function() {
    var packages = [
      new qx.test.io.part.MockPackage("a"),
      new qx.test.io.part.MockPackage("b", 0, true),
      new qx.test.io.part.MockPackage("c")
  ...
  it("load part with several packages including wrapped", function() {
      var packages = [
